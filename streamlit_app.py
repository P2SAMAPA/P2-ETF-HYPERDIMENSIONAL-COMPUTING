...
st.set_page_config(page_title="Hyperdimensional Computing", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🧠 Hyperdimensional Computing</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">10,000‑dim holographic vectors | Binding, bundling, permutation | Similarity‑based scoring</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 HD Computing")
...
st.sidebar.markdown(f"**Vector dim:** {config.VECTOR_DIM} | **Levels:** {config.NUM_LEVELS}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'hd_computing_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>HD score: {etf['hd_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized HD Score"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized HD Score", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🧠 Top ETFs by Hyperdimensional Computing Score (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Hyperdimensional computing** uses high-dimensional (10,000‑dim) holographic vectors.
        - Operations: **binding** (multiplication), **bundling** (majority sum), **permutation** (circular shift).
        - Market states are encoded as hypervectors using quantized returns and macro variables.
        - The score is the **similarity** (cosine) between the current state and the learned "up" prototype.
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the HD scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Hyperdimensional Computing | Holographic vectors for market state encoding")
